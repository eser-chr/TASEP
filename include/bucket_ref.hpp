...
public:
    ...
    bucket_ref(int ROWS, int COLS, const std::vector<T> &other) : _ROWS(ROWS), _COLS(COLS), _vector(other)
    {
        _size = ROWS * _COLS;
    ...
    int find_upper_bound(const T &val) const
    {
        ...
        VAL_CHECK(val < _p_cum_sums.back(), "In upper limit, the value passed is bigger or equal to the last element")
