...
@njit
def activation_to_factors(L: int, q: float, grid:np.ndarray,
                           grid_activated: np.ndarray) -> np.ndarray:
    """
    Calculate the multiplicative factor of kon based on the activation
     of a side.
    ...
    """
    A = np.ones(L) + (q - 1) * grid_activated
    A[np.where(grid!=0)]=0
...
def calc_propensities(L: int, grid: np.ndarray, grid_activated: np.ndarray, kon: float, koff: float, kstep: float, kq: float, q: float) -> np.ndarray:
    ...
    n_on = np.sum(activation_to_factors(L, q, grid, grid_activated))
    n_off = np.sum(grid)
...
def bind_kinesin(L: int, q: float, grid: np.ndarray, grid_activated: np.ndarray) -> None:
    ...
    Args:
        ...
        grid_activated (np.ndarray): 1D array representing the activated grid state.
    """
    S = np.cumsum(activation_to_factors(L, q, grid, grid_activated))
