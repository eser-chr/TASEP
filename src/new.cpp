...
template <typename T>
void fastTasep::BasicIteration<T>::printme() {
    std::cout << "kon: " << kon << "\n";
    std::cout << "koff: " << koff << "\n";
...
template <typename T>
void fastTasep::BasicIteration<T>::bind(int side) {
...
template <typename T>
void fastTasep::BasicIteration<T>::unbind(int side) {
...
template <typename T>
void fastTasep::BasicIteration<T>::step(int side) {
    temp = side * N_actions;
...
template <typename T>
void fastTasep::BasicIteration<T>::deactivate(int side) {
...
template <typename T>
void fastTasep::BasicIteration<T>::fix_boundaries() {
...
template <typename T>
void fastTasep::BasicIteration<T>::fix_cumsum(int side) {
...
template <typename T>
void fastTasep::BasicIteration<T>::iteration() {
    r1 = dis(gen);
...
void fastTasep::BasicIteration<T>::append_trajectory() {
    std::copy(grid.begin() + l_ghost, grid.end() - r_ghost, DATA.begin() + _iter * L);
    TIMES[_iter] = time;
    ACTION[_iter] = static_cast<u_int8_t>(_action);
    SIDE[_iter] = static_cast<u_int16_t>(_side - l_ghost);
};
template <typename T>
void fastTasep::BasicIteration<T>::simulation() {
